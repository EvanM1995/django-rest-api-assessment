...
class ViewSong(ViewSet):
    ...
    def create(self, request):
        ...
        song = Song.objects.create(
          ...
          album=request.date["album"],
          length=parse_duration(request.date["length"]),
          artist_id=artist_id,
        )
        serializer = SongSerializer(song)
        return Response(serializer.data)
    ...
    def update(self, request, pk):
        ...
        song = Song.objects.get(pk=pk)
        song.title = request.date["title"]
        song.album = request.date["album"]
        song.length = parse_duration(request.date["length"])
